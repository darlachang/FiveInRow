...
public abstract class MinMaxAI extends Controller {
	...
	protected @Override Location nextMove(Game g) {
		return minmax(g.getBoard(), me, depth).move;
	}

	private Node minmax(Board curBoard, Player nextPlayer, int depth){
		if (depth == 0 || curBoard.getState() != State.NOT_OVER)
			return new Node(estimate(curBoard), null);
		if (nextPlayer == me) {
			int maxScore = Integer.MIN_VALUE;
			Location bestMove = null;
			for (Location childMove : moves(curBoard)) {
				Board newBoard = curBoard.update(nextPlayer, childMove);
				int childScore = minmax(newBoard, nextPlayer.opponent(), depth - 1).score;
				if (maxScore < childScore) {
					maxScore = childScore;
					bestMove = childMove;
				}
			}
			return new Node(maxScore, bestMove);
		}
		else {
			int minScore = Integer.MAX_VALUE;
			Location worstMove = null;
			for (Location childMove : moves(curBoard)) {
				Board newBoard = curBoard.update(nextPlayer, childMove);
				int childScore = minmax(newBoard, nextPlayer.opponent(), depth - 1).score;
			...
			return new Node(minScore, worstMove);
		}
	}
